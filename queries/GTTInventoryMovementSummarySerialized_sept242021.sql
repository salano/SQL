...
from cin_hqtx
join csy_mbrc from_branch on from_branch.mbrc_branch = cin_hqtx.from_branch
join cin_iven on cin_iven.inv_number = cin_hqtx.inv_number
left outer join cin_lcst on cin_lcst.inv_number = cin_iven.inv_number and cin_lcst.branch = cin_hqtx.from_branch
left outer join cin_hlnk on cin_hlnk.wcon_id = (select wcon_id from cin_hlnk where refund_series = CONCAT(cin_hqtx.series_1, cin_hqtx.series_2) and refund_fol_number = cin_hqtx.fol_number limit 1)
left outer join tran_header on tran_header.series = CONCAT(cin_hqtx.series_1,cin_hqtx.series_2) and tran_header.folio_number = cin_hqtx.fol_number
where date_f between unix_timestamp("2020-11-01 00:00:00") and unix_timestamp("2020-12-01 00:00:00")
and cin_hqtx.ser_number != "(NULL SERIAL)"
and cin_iven.open_dept != 'Y'
and cin_hqtx.from_loc != '-'
...
join csy_mbrc from_branch on from_branch.mbrc_branch = cin_hqtx.from_branch
join cin_iven on cin_iven.inv_number = cin_hqtx.inv_number
join cin_lcst on cin_lcst.inv_number = cin_iven.inv_number and cin_lcst.branch = cin_hqtx.from_branch
left outer join tran_header on tran_header.series = CONCAT(cin_hqtx.series_1,cin_hqtx.series_2) and tran_header.folio_number = cin_hqtx.fol_number
where date_f between unix_timestamp("2020-11-01 00:00:00") and unix_timestamp("2020-12-01 00:00:00")
and cin_hqtx.ser_number != "(NULL SERIAL)"
and cin_iven.open_dept != 'Y'
